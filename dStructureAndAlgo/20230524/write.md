...
얼마나 걸리는가가 아닌 로직을 처리하는데 얼마나 함수가 실행되느냐?
...
5. hashTable
    ...
    일반적으로 랜덤한 장소에 저장하기 위해 난수화합니다.
    그러나 해시 테이블에서 저장되는 순서는 보장되지 않습니다. 해시 함수의 결과에 따라 키-값 쌍이 메모리에 저장되는 위치가 결정되기 때문에 저장 순서를 제어할 수 없습니다. 이는 해시 테이블이 자료 구조의 특성입니다.

    따라서 해시 테이블에서 저장된 값들을 순서대로 가져오는 작업은 보장되지 않습니다. 해시 테이블은 주로 특정 키를 기반으로 값을 검색하거나 업데이트하는 데 사용되며, 저장 순서에 대한 의존성은 없습니다. 값의 순서를 중요시하는 경우라면 다른 자료 구조를 사용해야 합니다.

---

## hashTable

해시 테이블은 해시 함수를 사용하여 키와 해당 키의 값 사이에 일대일 관계를 생성합니다. 따라서 키를 해시 함수에 적용하면 해당 키의 인덱스를 빠르게 계산할 수 있으며, 이를 통해 상수 시간 O(1) 내에 특정 키에 대한 값을 찾을 수 있습니다. 이는 대량의 데이터에서도 빠른 검색과 접근을 가능하게 합니다.

### 서로다른 키들이 동일한 hash값을 가지는 경우

해시 테이블은 유한한 공간에 데이터를 저장하는 자료 구조이기 때문에 충돌(Collision)이 발생할 수 있습니다. 충돌은 서로 다른 키가 동일한 해시 값으로 매핑되는 경우에 발생합니다. 이는 해시 함수의 특성상 피할 수 없는 현상입니다.

이러한 충돌이 일어나면 hashTable의 충돌을 일으킨다

중복이되면 기존의 값을 덮어씌우지않고 링크(링크 리스트)를 만들어서 계속해서 탐색할 수 있게한다
따라서, 값을 다루려면 값이 중복된 주소에 접근해서 추가적으로 탐색을 해야한다
그래서 성능이 저하된다

#### 참고

충돌이 발생하면 일반적으로 링크(링크 리스트)를 사용하여 중복된 주소에 값들을 연결하는 방식을 채택합니다. 이를 체이닝(Chaining)이라고도 합니다.

중복된 주소에 값을 추가하기 위해서는 해당 주소에 대한 링크 리스트를 탐색해야 합니다. 이는 값의 추가, 검색 또는 삭제를 수행할 때 추가적인 탐색 작업을 필요로 하게 됩니다. 즉, 같은 해시 값에 대해 연결된 링크 리스트를 따라가야 하는 과정이 필요합니다.

이로 인해 해시 테이블의 성능이 저하될 수 있습니다. 특히 충돌이 빈번하게 발생하고 링크 리스트의 길이가 길어지면 성능 저하가 더욱 심해질 수 있습니다. 값의 추가, 검색 또는 삭제 작업이 링크 리스트의 길이에 비례하는 시간이 소요되기 때문입니다.

성능 저하를 최소화하기 위해 충돌을 최소화하는 좋은 해시 함수의 선택이 중요합니다. 또한 충돌 해결 방법과 링크 리스트의 길이를 적절히 관리하여 해시 테이블의 성능을 향상시킬 수 있습니다. 충돌을 최소화하고 링크 리스트의 길이를 최대한 짧게 유지하는 것이 중요합니다.
